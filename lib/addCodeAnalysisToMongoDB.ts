...
export const addCodeAnalysisToMongoDB = async (data: assesment) => {
    try {
      const {
        code,
        comments,
      ...
      if (existingCodeAnalysis) {
        ...
        console.log(`Code '${code}' is already associated with bankId '${bankId}'.`);
        return {message:`Code '${code}' is already associated with bankId '${bankId}'.`};
      }
